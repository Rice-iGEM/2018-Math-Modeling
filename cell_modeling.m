...
function [P,Y] = initialize(type)
    ...
    %   Arguments "type" describes what to add ("plain"/"circuit"/"oribo"/"both inactive"/"both active")
    ...
    if type == "both inactive" % Add both
        %% Constants related to circuit genes
        w_Y = 100; % max circuit genes transcription - selected at random/may vary
        o_Y = 4.38; % transcription energy threshold for circuit genes
        b_Y = 1; % mRNA-ribosome binding rate
...
function dY = cell_wildtype_ODE(T,Y,P)
    %% cellODE(T,Y,P):
...
function dY = cell_circuit_ODE(T,Y,P)
    %% cell_circuit_ODE(T,Y,P):
    %   Calculates derivative with respect to time of all the variable in
    %   the simulation using equations from the Nat. Comm. 2018 paper
...
function dY = cell_oribo_ODE(T,Y,P)
    %% cell_oribo_ODE(T,Y,P):
    %   Calculates derivative with respect to time of all the variable in
    %   the simulation using equations from the Nat. Comm. 2018 paper
...
function dY = cell_both_inactive_ODE(T,Y,P)
    %% cell_both_inactive_ODE(T,Y,P):
    %   Calculates derivative with respect to time of all the variable in
    %   the simulation using equations from the Nat. Comm. 2018 paper
    %   Both circuit genes and o-ribosomes included
    %   Circuit genes only translated by host ribosomes
...
function dY = cell_both_active_ODE(T,Y,P)
    %% cell_both_active_ODE(T,Y,P):
    %   Calculates derivative with respect to time of all the variable in
    %   the simulation using equations from the Nat. Comm. 2018 paper
    %   Both circuit genes and o-ribosomes included
    %   Circuit genes are only translated by o-ribosomes
...
function Yf = runToSteadyState()
    %% runToSteadyState()
    % Runs cell_wildtype_ODE() until E_cell stabilizes and output only the final values.
...
function [T,Y] = runAfterSteadyState(type, tmax)
    %% runAfterSteadyState(type, tmax)
    % Initialize and run cell_wildtype_ODE() to get steady state in the absence of other genes.
    % Then, run the ODE described in type ("plain"/"circuit"/"oribo"/"both inactive"/"both active")
    % tmax gives the maximum time to run till
    [P,Y] = initialize(type); % Initialize variables/parameters
    Y0 = runToSteadyState(); % Run until steady state and use as initial condition
    switch type
        case "circuit"
        ...
        case "oribo"
            ODE = @cell_oribo_ODE;
        case "both inactive"
            ODE = @cell_both_inactive_ODE;
        case "both active"
            ODE = @cell_both_active_ODE;
        otherwise
            ODE = @cell_wildtype_ODE;
    end
    for i = [length(Y0)+1:length(Y)]  % Add any remaining initial variables required for the ODE to Yf
        Y0 = [Y0 Y(i)];
    end
    odeoptions = odeset('NonNegative',[1:length(Y0)]);
    [T,Y] = ode15s(@(T,Y) ODE(T,Y,P), [0 tmax], Y0, odeoptions); % Run the correct ODE
end

function Yf = varyParameters(type, par_num, Ps)
    %% varyParameters(type, par_num, Ps)
    % Returns steady-state values of variables for different values of
